...
class TestTransferClientPortfolio(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.context = {
            "current_salesperson_id": cls.demo_user.id,
            "new_salesperson_id": cls.adm_user.id,
            "review_state": True,
            "contact_ids": cls.partner.ids,
            "activity_ids": [cls.opor_activity.id, cls.part_activity.id],
            "opportunity_ids": [cls.lead.id],
    ...
    def test_transfer_client_portfolio(self):
        ...
        self.assertEqual(self.part_activity.user_id.id, self.demo_user.id)

        self._execute_transfer(self.context)
        ...
        self.assertEqual(self.part_activity.user_id.id, self.adm_user.id)

        self.context = {
            "current_salesperson_id": self.adm_user.id,
            "new_salesperson_id": self.demo_user.id,
            "update_salesperson_contact": False,
            "contact_ids": self.partner.ids,
            "activity_ids": [self.opor_activity.id, self.part_activity.id],
        ...
        self.part_activity.write({"user_id": self.demo_user.id})
        ...
        self.context = {
            "current_salesperson_id": self.demo_user.id,
            "new_salesperson_id": self.adm_user.id,
            "contact_ids": self.partner.ids,
            "activity_ids": [self.opor_activity.id, self.part_activity.id],
            "opportunity_ids": [self.lead.id],
