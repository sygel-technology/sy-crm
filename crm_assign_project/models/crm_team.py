...
class CrmTeam(models.Model):
    ...
    def write(self, vals):
        ...
        if (
            'automatic_project_assignation' in vals or 'automatic_assignation_project_id' in vals
        ) and self.alias_id:
            alias_values = self.get_alias_values()
            self.alias_id.write({
                'alias_defaults': alias_values['alias_defaults']
            })            
        return values
    ...
    def get_alias_values(self):
        ...
        if self.automatic_project_assignation and self.automatic_assignation_project_id:
            values['alias_defaults']['project_id'] = self.automatic_assignation_project_id.id
